...
switch ($tag) {
    case 'home':
        $data = $conn->query("SELECT * FROM flashsale WHERE status_tampil_waktu = 'Y' AND status_remove_flashsale = 'N' AND waktu_mulai <= NOW() AND waktu_selesai >= NOW()");
        foreach ($data as $key => $value) {
            ...
            LEFT JOIN variant d ON c.id_master = d.id_master
            ...
            foreach ($dataproduct as $key => $key2) {
                ...
                $data_produk[] = [
                    ...
                    'slug_judul_master' => $key2['slug_judul_master'],
