...
foreach ($getproduk as $u) {
    ...
    if (($u->id_variant != null or !empty($u->id_variant))) {
        ...
        $dataproduct = $conn->query("SELECT *, (stok_flashdisk-stok_terjual_flashdisk) as sisa_stok FROM flashsale a 
                ...
                WHERE status_tampil_waktu = 'Y' AND status_remove_flashsale = 'N' AND a.waktu_mulai <= CURRENT_DATE AND a.waktu_selesai >= CURRENT_DATE AND b.kd_barang = '$u->id_master'")->fetch_object();
